...
if (fs.existsSync(configFilePath)) {
  ...
  // Default values
  config = JSON.parse(
    '{ "httpPort": 8080, "websocketPort": 3476, "websocketIp": "localhost" }'
  );
}
...
if (typeof config.hrImageFile !== "undefined") {
  config.hrImage =
    ...
    base64_encode(path.dirname(process.execPath) + "/" + config.hrImageFile);
}
if (typeof config.calImageFile !== "undefined") {
  config.calImage =
    "data:image/png;base64, " +
    base64_encode(path.dirname(process.execPath) + "/" + config.calImageFile);
}
...
request("https://git.io/JJ8uD", { json: true }, (err, res, body) => {
  ...
  if (semver.gt(body.version, version)) {
    console.log(
      "There is a new version available! Download it here: https://git.io/JJ8uR"
    );
  } else if (semver.gt(version, body.version)) {
    console.log(
      "You are running an unreleased version. Feeling adventurous, are we?"
    );
  } else {
...
wss.on("connection", function connection(ws) {
  ws.on("message", function incoming(message) {
    if (message === "webClient") {
      webClients.push(ws);
      console.log(
        "WebSocket web client connected (" + ws._socket.remoteAddress + ")"
      );
    } else {
...
function sendDataToWebClients(data) {
  ...
  webClients
    ...
    .forEach((disconnectedClient) => {
      console.log(
        "WebSocket client disconnected (" +
          disconnectedClient._socket.remoteAddress +
          ")"
      );
